...
def test_send_multicast_startup_discover_multicast():
    ...
    assert str(return_val) == "{'username': 'Tobias'}" 
...
def test_send_multicast_load_local_data():
    local_data = send_multicast.load_local_data()
    assert str(local_data) == "{'username': 'Tobias'}"
